...
| `matrixDomain` | str | *host of `matrixUrl`* | Matrix server name used to build ghost mxids (`@_jmap_…:<domain>`). Set explicitly when the API host differs from the `server_name`. |
...
> Ghost mxids (`@_jmap_…:<domain>`) use `matrixDomain`, which defaults to the host
> of `matrixUrl`. Since `matrixUrl` is usually a loopback address, set `matrixDomain`
> explicitly to your homeserver's `server_name` (e.g. `example.com`) in real
> deployments.
