...
* **Oracles — `is_ok()` is not one.** This bridge degrades gracefully **on purpose**: a failed send notifies the user instead of crashing (`src/commands/email.rs`), and `poll()` swallows per-email errors so one bad message can't stall a sync (`src/sync/email.rs`). Right for production — but it means `assert!(result.is_ok())` on those paths passes **by construction** and asserts nothing. Anywhere the code under test swallows its own errors, the mock is the only available oracle:
    * Pin every request the behaviour must make with **`.expect(N)`** (or assert on `received_requests()`), and assert the **user-visible outcome** — the notification text, the message body that reached the room.
    * A `Mock` mounted **without** `.expect(N)` is never verified: if the code stops making that request entirely, the test still passes. Unpinned mocks are fine as *supporting stubs*; they are not evidence.
    * Use **`.expect(0)`** to assert a request must NOT happen. That is what makes a negative case (e.g. "an empty mailbox bridges nothing") a real property rather than dead mocks.
* **Prove a test can fail.** Before committing a new or changed test, break the code it covers and confirm it goes **red**; then revert. A test you cannot make fail on demand is not testing anything, and this suite has shipped several — `test_full_bridge_cycle` stayed green while the entire `!email` send path was deleted. See `tests/bridge_test.rs` and `tests/event_source_test.rs` for the two shapes (positive `.expect(1)` and negative `.expect(0)`).
* **Never exclude a test from the gate to make it pass.** `default.nix`'s nextest args deliberately carry **no** `-E` filter: every test is hermetic, so the sandbox runs all of them. An exclusion hid a genuinely broken `test_sender_flow` on `main` for five weeks. If a test truly cannot run in the sandbox, say which and why in a comment.
...
* **Merge every PR so it fast-forwards `main`.** release-plz builds the changelog by walking a *single* ancestry chain, so a merge whose branch was behind `main` silently drops the other parent's commits from `CHANGELOG.md` and the Release notes — it cost four releases before anyone noticed. Merge commits are **disabled on the repo** for this reason; squash (the default) and rebase both fast-forward, so the walk can't fork. The `changelog-guard` job in `release-plz.yml` replays that walk and goes red when the release window contains a commit the changelog will miss; run it locally with `just changelog-guard`. See [ADR-0008](docs/adr/0008-ci-and-release-flow.md).
* **Write the PR title as a Conventional Commit subject.** With squash merges, a multi-commit PR takes its commit subject from the **PR title** (`squash_title: COMMIT_OR_PR_TITLE`), and that subject is exactly what release-plz puts in `CHANGELOG.md` and the GitHub Release body. A single-commit PR keeps its own subject, so the title only bites on the multi-commit ones — where a vague title ships verbatim in the release notes.
