...
* **Merge every PR so it fast-forwards `main`** (squash/rebase, or update the branch first). release-plz builds the changelog by walking a *single* ancestry chain, so a merge whose branch was behind `main` silently drops the other parent's commits from `CHANGELOG.md` and the Release notes — it has cost two releases already. The `changelog-guard` job in `release-plz.yml` replays that walk and goes red when the release window contains a commit the changelog will miss; run it locally with `just changelog-guard`. See [ADR-0008](docs/adr/0008-ci-and-release-flow.md).
